...
def main():
	...
	parser.add_argument("min_coverage", type=int, help="minimum fraction of the query hit by the alignment needed to annotate")
	...
	with open(output_file, "w") as output_handle:
	    for record in record_dict:
	        ...
	        if len(v) > 1:
	            for res in v:
	                hit_cov = int(res.length)/int(res.qlen)
	                if check_hit_position(BlastRes_obj, coverage,
